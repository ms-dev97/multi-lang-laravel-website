...
@section('content')
    <div class="search-page">
        <div class="container">
            <h1 class="page-title">
                {{ __('app.searching_for') }}: {{ $q }}
            </h1>

            <section class="results my-5">
                @forelse ($results as $result)
                    <div class="result d-flex flex-sm-row flex-column gap-3 mb-4">
                        <a href="{{ $result->showUrl }}" class="thumbnail">
                            <img src="{{ getImgThumbnail($result->image) }}" alt="">
                        </a>
            ...
            {{ $results->withQueryString()->links('pagination::default') }}
        </div>
    </div>
@endsection
